"""Tests for neverend CLI entry point."""
...
def test_help():
    ...
    assert 'neverend' in r.stdout.lower() or 'usage' in r.stdout.lower() or len(r.stdout) > 20
...
def test_health():
    """health command should run without crash."""
    r = subprocess.run([sys.executable, os.path.join(SCRIPTS, 'cli.py'), 'health'],
                       capture_output=True, text=True, timeout=10)
    assert r.returncode == 0
